...
namespace DocucenterBFA.Controllers
{
  ...
  //]";
        ...
        // Acción para consultar todos los hashes
        [HttpGet]
        public async Task<JsonResult> GetHashes()
        {
            try
            {
                decimal balance = await this.GetBalance();
        ...
        public async Task<JsonResult> StoreHash([FromBody] HashInput input)
        {
            try
            {
                ...
                // Verificar si el hash ya existe antes de almacenarlo
                var checkHashFunction = contract.GetFunction("checkHash");
                bool exists = await checkHashFunction.CallAsync<bool>(hashHex);
                if (exists)
                {
                    return Json(new { success = false, message = "El hash se encuentra registrado en la BFA." });
                }
        ...
        public async Task<JsonResult> GetHashData([FromQuery] string hash)
        {
            try
            {
                ...
                // Recuperar el firmante (dirección pública)
                ...
                var responseData = new
                {
                    ...
                    idTabla = result.IdTablas != null && result.IdTablas.Count > 0 ? result.IdTablas[0].ToString() : "No registra", // Extrae el primer valor de idTabla
                    nombreTabla = result.NombreTablas != null && result.NombreTablas.Count > 0 ? result.NombreTablas[0] : "No registra", // Extrae el primer valor de nombreTabla
        ...
        private async Task<decimal> GetBalance()
        {
            ...
            var balanceWei = await web3.Eth.GetBalance.SendRequestAsync("0x6ca44cc5CDBB8426812b163A7bE47600F591989e");
